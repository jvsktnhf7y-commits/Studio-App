...
export default function LoginScreen({ navigation }) {
  ...
  async function handleLogin() {
    ...
    setLoading(true);
    try {
      const res = await login(name.trim(), accessCode.trim());
      if (res.ok) {
  ...
  return (
    <LinearGradient colors={['#48bb78', '#667eea']} style={styles.gradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.card}>
            ...
            <Text style={styles.cardSub}>Enter your name and access code from your teacher</Text>
            ...
            <TextInput style={styles.input} value={accessCode} onChangeText={setAccessCode} placeholder="e.g. MUSIC123" placeholderTextColor={COLORS.muted} autoCapitalize="characters" returnKeyType="done" onSubmitEditing={handleLogin} />
