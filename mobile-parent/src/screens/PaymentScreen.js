...
import { createPaymentIntent, getDashboard } from '../services/api';
import { COLORS, GRADIENT } from '../theme';

const PAYMENT_METHODS = [
  { id: 'card',   label: '💳  Pay by Card',   subtitle: '$3.00 convenience fee' },
...
export default function PaymentScreen({ navigation }) {
  ...
  useEffect(() => {
    getDashboard().then(d => { if (d.ok) setBalance(d.balance); }).catch(() => {});
  }, []);
  ...
  async function handleCardPayment() {
    ...
    setLoading(true);
    try {
      const res = await createPaymentIntent(Math.round(amt * 100), 'Lesson payment');
      if (!res.ok) { Alert.alert('Error', res.error || 'Could not create payment.'); return; }

      const { error: initError } = await initPaymentSheet({
        merchantDisplayName: 'Music Studio',
      ...
      <View style={styles.successContainer}>
        ...
        <View style={styles.successBody}>
          <View style={styles.resultRow}>
            ...
            <Text style={[styles.resultValue, { color: COLORS.success }]}>${paid.amount.toFixed(2)}</Text>
          </View>
          {paid.fee > 0 && (
            <View style={styles.resultRow}>
              <Text style={styles.resultLabel}>Convenience fee</Text>
              <Text style={styles.resultValue}>${paid.fee.toFixed(2)}</Text>
            </View>
          )}
          <View style={[styles.resultRow, { borderBottomWidth: 0 }]}>
            <Text style={styles.resultLabel}>Method</Text>
            <Text style={styles.resultValue}>{paid.method.charAt(0).toUpperCase() + paid.method.slice(1)}</Text>
          </View>
      ...
      <ScrollView style={styles.container} contentContainerStyle={{ padding: 16, paddingBottom: 48 }} keyboardShouldPersistTaps="handled">
        ...
        {method === 'card' && (
          <View style={styles.feeNotice}>
            <Text style={styles.feeText}>A $3.00 convenience fee will be added for online card payments.</Text>
          </View>
        )}

        {method !== 'card' && (
          <View style={styles.feeNotice}>
            <Text style={styles.feeText}>No fee for {method} — pay your teacher directly, then tap below to record it.</Text>
          </View>
        )}

        <Button
          label={method === 'card' ? `Pay $${((parseFloat(amount) || 0) + 3).toFixed(2)} by Card →` : `Record ${method.charAt(0).toUpperCase() + method.slice(1)} Payment →`}
